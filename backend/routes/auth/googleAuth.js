...
router.get('/callback', (req, res, next) => {
  ...
  passport.authenticate('google', { failureRedirect: '/login?error=google-auth-failed' }, (err, user) => {
    ...
    if (!user) {
      ...
      return res.redirect(`${process.env.FRONTEND_URL || 'http://localhost:5173'}/login?error=auth-failed`);
    }
    ...
    res.redirect(`${process.env.FRONTEND_URL || 'http://localhost:5173'}/auth/success?token=${token}&userId=${user._id}&email=${encodeURIComponent(user.email)}&name=${encodeURIComponent(user.name)}&newUser=${user.newUser}&profileCompleted=${user.profileCompleted}&userType=${encodeURIComponent(user.userType || 'user')}`);
  })(req, res, next);
});
